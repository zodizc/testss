...
node {
	...
	stage('Deploye Code') {
		...
		if (isUnix()) {
			...
			rms = bat returnStdout: true, script:"${toolbelt} force:apex:test:run --classnames TemperatureConverterTest"
			ind = rms.indexOf("-i")+3
			fid = rms.substring(ind)
			id = fid.split(" ")[0]
			println(rms)
			rmsr = bat returnStdout: true, script:"${toolbelt} force:apex:test:report -i " +id+" -c -u \"mfarouq@leyton.com.devadmin\" -r junit"
