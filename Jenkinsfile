	...
	withCredentials([file(credentialsId:'5ad9e141-21dc-42fe-afb2-b79dd63e6eb8', variable:'jwt_key_file')]) {
		stage('Create ScratchOrg'){
			if (isUnix()) {
				...
				scratchorg = bat returnStatus: true, script: "${toolbelt} force:org:create --targetdevhubusername SandBox --setdefaultusername --definitionfile config/project-scratch-def.json --setalias ciorg --wait 10 --durationdays 1"
				if (scratchorg != 0) {
					error 'Salesforce test scratch org creation failed.'
				}
			}
		...
		stage('Run Tests in ScratchOrg'){
			if (isUnix()) {
				...
				testres = bat returnStdout: true, script: "${toolbelt} force:apex:test:run --targetusername ciorg -l RunAllTestsInOrg -w 20 -u mafarouq@leyton.com"
			}
		}
